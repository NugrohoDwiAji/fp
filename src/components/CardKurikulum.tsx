...
export default function CardKurikulum({ children, semester }: Props) {
  ...
  return (
    <div className={`bg-white max-w-sm md:max-w-xl lg:max-w-3xl relative shadow-2xl  rounded-lg ${isOpen?" border-2 border-collapse  border-blue-300":""} mb-5 transform `}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="p-2 cursor-pointer outline-none text-lg font-bold mb-1 w-full rounded-lg ring-2 ring-blue-500 flex items-center justify-between"
      >
        {"Semester " + semester}
        <button
          onClick={() => setIsOpen(!isOpen)}
      ...
      <div
        className={` ${isOpen?" m-2 md:m-5 bg-white rounded-2xl shadow-lg border border-blue-200 overflow-hidden":"m-0 hidden"} `}
      >
        <div
